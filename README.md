...
The review finds that self-representation, traditionally studied across 'levels' of self and interactive complexity (embodied, affective, agentic, social, narrative), appear to share a common computational goal : encoding the agent's position within the causal structure it perceives in its environment. This is hypothesised to develop primarily through patterns of contingency - causal relationships whose origin can reliably be attributed to the self.

The review seems to point towards a layered inferential process, combining multisensory integration, causal inference, and perception of contingency. This experiment questions this potential mechanism by hypothesising that contingency is encoded through statistical learning within its environment : what properties co-occur reliably with self-indexed causality? When does the agent hold a distinct position in the causal network it observes? 
...
Press rates, learning, and relationships between variables were assessed, alongside 4 candidate computational models. Details can be found below, with most analysis living in the dedicated `notebooks/` folder. I used several candidate models to evaluate the pre-registered hypotheses, and implemented the PsyTrack toolbox to look at trial-by-trial fluctuations in weights and learning trajectories, as proposed in *Extracting the dynamics of behavior in sensory decision-making experiments* (Roy et al, 2021).
